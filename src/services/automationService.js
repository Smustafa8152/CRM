class AutomationService {
    ...
    checkOverduePayments(invoices, customers) {
        const today = new Date();
        const overdueInvoices = invoices.filter(invoice => {
            if (!invoice.creditSale || invoice.status === 'paid')
                return false;
    ...
    checkLowStock(inventory) {
        const lowStockItems = inventory.filter(item => item.quantity <= 10 && item.stockStatus !== 'Out of Stock');
        lowStockItems.forEach(item => {
            this.sendLowStockAlert(item);
        });
    }
    // Check for warranty expiry
    checkWarrantyExpiry(invoices) {
        const today = new Date();
    ...
    sendPaymentReminder(customer, invoice) {
    ...
    sendWarrantyExpiryNotification(invoice, item) {
    ...
    calculateCreditScore(customer, invoices) {
        ...
        if (outstandingPercentage > 80)
            score -= 20;
        else if (outstandingPercentage > 50)
            score -= 10;
        // Add points for early payments
        const earlyPayments = customerInvoices.filter(inv => inv.paymentHistory?.some(payment => {
    ...
    suggestCreditLimit(customer, invoices) {
    ...
    calculateMonthlyAverages(invoices) {
